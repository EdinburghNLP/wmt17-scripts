...
THEANO_FLAGS=mode=FAST_RUN,floatX=float32,device=$device python $nematus_home/nematus/translate.py < $tmpfile_src \
     ...
     -k 50 -p 2 --n-best > $tmpfile_nbest

#rescoring
$model_dir/../scripts/reverse_nbest.py < $tmpfile_nbest | \
GPUARRAY_FORCE_CUDA_DRIVER_LOAD=True THEANO_FLAGS=mode=FAST_RUN,floatX=float32,device=$device,gpuarray.preallocate=0.8 python $nematus_home/nematus/rescore.py  \
     -m $model_dir/model.r2l.ens{1,2,3,4}.npz \
     -b 40 -s $tmpfile_src  | \
$model_dir/rerank_normalize.py 50 1 | \
